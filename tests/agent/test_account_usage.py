...
def test_codex_usage_falls_back_to_native_credential_pool(monkeypatch, codex_usage_payload):
    ...
    pool_token = _codex_jwt("acct_pool_123")
    pool_entry = SimpleNamespace(
        runtime_api_key=pool_token,
    ...
    assert calls[0]["url"] == "https://chatgpt.com/backend-api/wham/usage"
    assert calls[0]["headers"]["Authorization"] == f"Bearer {pool_token}"
    assert calls[0]["headers"]["ChatGPT-Account-Id"] == "acct_pool_123"


def test_codex_usage_derives_account_id_from_resolved_pool_token(
    ...
    pool_token = _codex_jwt("acct_resolved_pool_456")
    monkeypatch.setattr(
        account_usage,
        "resolve_codex_runtime_credentials",
        lambda **kwargs: {
